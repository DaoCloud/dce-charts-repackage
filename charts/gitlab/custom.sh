...
# the old bitnami-shell is deprecated, if gitlab chart using a correct image and tag
# we can remove this workaround
OS_SHELL_VERSION=11-debian-11-r112
...
yq -i '
  ...
  .gitlab.postgresql.image.repository = "bitnamilegacy/postgresql" |
  .gitlab.postgresql.volumePermissions.image.registry = "docker.m.daocloud.io" |
  .gitlab.postgresql.volumePermissions.image.repository = "bitnamilegacy/os-shell" |
  .gitlab.postgresql.volumePermissions.image.tag = "'$OS_SHELL_VERSION'" |
  .gitlab.postgresql.metrics.image.registry = "docker.m.daocloud.io" |
  .gitlab.postgresql.metrics.image.repository = "bitnamilegacy/postgres-exporter" |
  .gitlab.redis.image.registry = "docker.m.daocloud.io" |
  .gitlab.redis.image.repository = "bitnamilegacy/redis" |
  .gitlab.redis.metrics.image.registry = "docker.m.daocloud.io" |
  .gitlab.redis.metrics.image.repository = "bitnamilegacy/redis-exporter" |
  .gitlab.redis.sentinel.image.registry = "docker.m.daocloud.io" |
  .gitlab.redis.sentinel.image.repository = "bitnamilegacy/redis-sentinel" |
  .gitlab.redis.volumePermissions.image.registry = "docker.m.daocloud.io" |
  .gitlab.redis.volumePermissions.image.repository = "bitnamilegacy/os-shell" |
  .gitlab.redis.volumePermissions.image.tag = "'$OS_SHELL_VERSION'" |
  .gitlab.redis.sysctl.image.registry = "docker.m.daocloud.io" |
  .gitlab.redis.sysctl.image.repository = "bitnamilegacy/os-shell" |
  .gitlab.redis.sysctl.image.tag = "'$OS_SHELL_VERSION'" |
...
echo "remove !!merge generated by yq"
sed -i 's/!!merge <</<</' values.yaml
sed -i 's/!!merge <</<</' charts/gitlab/values.yaml
