...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
        - name: {{ .name }}
          image: '{{ if and .imageOverride.registry .imageOverride.repository}}{{ .imageOverride.registry }}/{{ .imageOverride.repository }}{{ else }}{{ .defaultValues.image.repository }}{{ end }}:{{ .imageOverride.tag | default (printf "v%s-%s" (default .Chart.AppVersion .defaultValues.image.tag) (replace "-" "" .name)) }}'
