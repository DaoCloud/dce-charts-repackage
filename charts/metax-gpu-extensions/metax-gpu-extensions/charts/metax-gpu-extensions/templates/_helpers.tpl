...
{{- define "GPUExt.topoMasterSboxCmd" -}}
- --mode=switchbox
- --cluster-manager-address={{ ((.Values.topoDiscovery).switchbox).clusterManagerAddress }}
- --cluster-manager-poll-topo-period={{ default "30" ((.Values.topoDiscovery).switchbox).clusterManagerPollTopoPeriod }}
- --cluster-manager-poll-event-period={{ default "10" ((.Values.topoDiscovery).switchbox).clusterManagerPollEventPeriod }}
{{- end }}

{{- define "GPUExt.topoWorkerSboxCmd" -}}
- --detect-mode=dragonfly
- --node-number={{ default "2" ((.Values.topoDiscovery).switchbox).nodeNumber }}
- --enable-training={{ default false ((.Values.topoDiscovery).switchbox).enableTraining }}
{{- end }}
