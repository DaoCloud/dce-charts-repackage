...
chart still renders on its own (helm unittest and `helm template` of this
directory both load the subchart without its parent).

They are deliberately named nic-health-monitor.* rather than nvsentinel.*:
...
*/}}
{{- define "nic-health-monitor.pcAuth.enabled" -}}
...
{{- define "nic-health-monitor.pcAuth.mountPath" -}}
...
{{- define "nic-health-monitor.pcAuth.tokenPath" -}}
{{- printf "%s/token" (include "nic-health-monitor.pcAuth.mountPath" .) -}}
{{- end -}}

{{- define "nic-health-monitor.pcAuth.volume" -}}
  ...
  projected:
    sources:
      - serviceAccountToken:
          ...
          expirationSeconds: {{ include "nic-health-monitor.pcAuth.expirationSeconds" . }}
          path: token
{{- end -}}

{{- define "nic-health-monitor.pcAuth.volumeMount" -}}
- name: platform-connector-token
  mountPath: {{ include "nic-health-monitor.pcAuth.mountPath" . }}
  readOnly: true
{{- end -}}

{{- define "nic-health-monitor.pcAuth.expirationSeconds" -}}
