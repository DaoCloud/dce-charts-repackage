...
yq e '
    .vgpu.devicePlugin.hygonImageRepository="4pdosc/vdcu-device-plugin" |
    .vgpu.devicePlugin.hygonImageTag="v1.0"
' -i values.yaml

yq e '
    .devicePlugin.hygonImageRepository="4pdosc/vdcu-device-plugin" |
    .devicePlugin.hygonImageTag="v1.0"
' -i charts/vgpu/values.yaml

# update daemonsethygon.yaml image
if [ $os == "Darwin" ];then
   sed -i "" "s/{{ .Values.devicePlugin.hygonimage }}/\"{{ .Values.devicePlugin.registry }}\/{{ .Values.devicePlugin.hygonImageRepository }}:{{ .Values.devicePlugin.hygonImageTag | default .Chart.AppVersion }}\"/g" ./charts/vgpu/templates/device-plugin/daemonsethygon.yaml
elif [ $os == "Linux" ]; then
   sed -i  "s/{{ .Values.devicePlugin.hygonimage }}/\"{{ .Values.devicePlugin.registry }}\/{{ .Values.devicePlugin.hygonImageRepository }}:{{ .Values.devicePlugin.hygonImageTag | default .Chart.AppVersion }}\"/g" ./charts/vgpu/templates/device-plugin/daemonsethygon.yaml
fi
