...
```console
helm install my-release oci://REGISTRY_NAME/REPOSITORY_NAME/wordpress --set "global.imagePullSecrets[0]=SECRET_NAME" -n NAMESPACE
```
...
When performing admin operations that require activating the maintenance mode (such as updating a plugin or theme), it's activated in only one replica. This implies that WP could be attending requests on other replicas while performing admin operations, with unpredictable consequences.
...
| `metrics.podAnnotations`                                    | Annotations for metrics exporter pod                                                                                                                                                                                              | `{}`                              |
...
| `mariadb.networkPolicy.enabled`                   | Enable NetworkPolicy for MariaDB                                                                                                                                                                                           | `true`              |
| `mariadb.networkPolicy.allowExternal`             | Don't require server label for connections                                                                                                                                                                                 | `false`             |
| `mariadb.networkPolicy.addExternalClientAccess`   | Allow access from pods with client label set to "true". Ignored if `mariadb.networkPolicy.allowExternal` is true.                                                                                                          | `false`             |
| `mariadb.networkPolicy.ingressReleaseMatchLabels` | Labels to match pods in the same release to allow traffic from                                                                                                                                                             | `{}`                |
...
This major release bumps the MariaDB version to 13.1. Follow the [upstream instructions](https://mariadb.com/docs/server/server-management/install-and-upgrade-mariadb/upgrading) for upgrading from MariaDB 13.0 to 13.1. No major issues are expected during the upgrade.
