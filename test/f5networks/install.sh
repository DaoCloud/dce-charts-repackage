...
if [ "${RUN_ON_LOCAL}" = "false" ]; then
  HELM_MUST_OPTION+=" --set f5-bigip-ctlr.image.user=docker.io/f5networks \
  --set f5-ipam-controller.image.user=docker.io/f5networks "
fi
...
if [ -z "${IMAGE_LIST}" ] ; then
  echo "warning, failed to find image from chart template for f5networks"
else
  echo "found image from f5networks chart template: ${IMAGE_LIST}"
  for IMAGE in ${IMAGE_LIST} ; do
    ...
    echo "load local image ${IMAGE} for f5networks"
