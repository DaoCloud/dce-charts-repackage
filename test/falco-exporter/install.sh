...
IMAGE_LIST=` helm template test daocloud/falco-exporter ${HELM_MUST_OPTION} | grep " image: " | tr -d '"'| awk '{print $2}' `
if [ -z "${IMAGE_LIST}" ] ; then
  echo "warning, failed to find image from chart template for falco-exporter"
else
  echo "found image from falco-exporter chart template: ${IMAGE_LIST}"
  for IMAGE in ${IMAGE_LIST} ; do
    ...
    echo "load local image ${IMAGE} for falco-exporter"
