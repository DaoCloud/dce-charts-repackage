...
if [ -z "${IMAGE_LIST}" ] ; then
  echo "warning, failed to find image from chart template for node-problem-detector"
else
  echo "found image from node-problem-detector chart template: ${IMAGE_LIST}"
  for IMAGE in ${IMAGE_LIST} ; do
    ...
    echo "load local image ${IMAGE} for node-problem-detector"
