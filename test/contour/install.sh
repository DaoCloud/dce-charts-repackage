...
if [ "${RUN_ON_LOCAL}" = "false" ]; then
  HELM_MUST_OPTION+=" --set global.imageRegistry=release.daocloud.io "
fi
...
if [ -z "${HELM_IMAGES_LIST}" ] ; then
  echo "warning, failed to find image from chart template for contour"
else
  echo "found image from contour chart template: ${HELM_IMAGES_LIST}"
  for IMAGE in ${HELM_IMAGES_LIST} ; do
    ...
    echo "load local image ${IMAGE} for contour"
